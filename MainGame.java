...
public class MainGame extends World
{
    ...
    {    
        ...
        addObject(boy,30, getHeight() - 60);
        addObject(healthbar, ((boy.getX() - (boy.getImage().getWidth()/2)) + (healthbar.getImage().getWidth()/2)) + 5, boy.getY() - (boy.getImage().getHeight()/2) - 5);
    ...
    {
        if (boy.CheckForLevelCompletion() == true) {
            if (currentLevel == 1) {
                ...
                newLevelSound.play();
                music.stop();
                Greenfoot.setWorld(new WinningScreen(getWidth(), getHeight(), totalTimeTaken, boyLifes));
            }
        }
    }
    ...
    public void act()
    {
        ...
        healthbar.setLocation(((boy.getX() - (boy.getImage().getWidth()/2)) + (healthbar.getImage().getWidth()/2)) + 5, boy.getY() - (boy.getImage().getHeight()/2) - 5);
    ...
    public void CheckForboyDeath()
    {
        if (boy.getHealth() <= 0 || (int)(timeLimit - time)/1000 < 1) {
            removeAllObjectsInWorld();
            boyLifes--;
            Reset();
            prepare();
    ...
    void removeAllObjectsInWorld()
    {
        ...
        for (int j = 0; j < 15; j++) {
            for (int i = 0; i < getObjects(Actor.class).size(); i++) {
                if (getObjects(Actor.class).get(i).getClass() != boy.class && getObjects(Actor.class).get(i).getClass() != Healthbar.class) {
    ...
    void DisplayboyLifes()
    {
        showText("Lifes: " + boyLifes, 400, 10);
    }
    
    void Reset()
    {
        boy.Reset();
        boy.setLocation(30, getHeight() - 60);
        healthbar.Reset();
        time = 0;
    }
    ...
    {
        showText("Coins: " + boy.coinsCollected, 50, 10);
    }
    
    public int getboyCoinsCollected()
    {
         return boy.getCoinsCollected();
    }
    ...
    public void UpdatePlayerLocationToEnemey()
    {
        ...
        while (i < getObjects(Enemy.class).size()) {
            getObjects(Enemy.class).get(i).UpdatePlayerPosition(boy.getX(), boy.getY());
            i++;
        }
    }
}
